...
for fn in fnames:
  ...
  maxx = max([k[0] for k in lst])  #sqrt(n)
  if n in {100}:
    pruned_lst = [(k[0], k[1]) for k in lst if k[0] <= maxx and k[0] >= minx]
    point_sequences[(n, fn)] = pruned_lst
...
for tup, seq in point_sequences.items():
  ...
  ax.plot([k[0] for k in seq], [k[1] for k in seq], '*', label='data for n = {}'.format(tup))
  ax.plot([k for k in poly_pts_x], [upper_bound(k) for k in poly_pts_x], label='upper bound for n = {}'.format(tup))
  ax.plot([k for k in poly_pts_x], [lower_bound(k) for k in poly_pts_x], label='lower bound for n = {}'.format(tup))
plt.xlabel('cache size')
plt.ylabel('cache misses')
t = 'KMP FF function, LRU, instance type: (a^(n-1))b'
t+= '\nconjecture: (cache misses)(cache size) is between n^2/2 and n^2/2 + n*sqrt(n) where cache size is between 1 and sqrt(n)'
t+= '\nseveral input types for n = 100'
